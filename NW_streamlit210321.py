...
def getData():
    ...
    df2 = df2[['tree_name','description','species', 'genus', 'family', 'address', 'ownership_code', 'location_code', 'native', 'crown_width','dbh_class', 'defects', 'diversity_level', 'latitude', 'longitude']]
...
def showTable(data, cols):
   ...
   # cols = data.columns
    ...
    fig.layout.title = 'Most of the Data'
...
def getSelection(data):
    #Filter datafraame values for mapping and analysis

    data_columns = data.columns.values.tolist()
    add_all = ['ALL']
    data_columns = add_all + data_columns
    select_param = ['ALL']
    select_param = st.sidebar.selectbox('Select a parameter for filtering', data_columns)    
    ...
    if select_param != 'ALL':
        ...
        qString = f"{select_param} in {value_list}"
        
        # select_df = data.query("@select_param == @select_value")
        #select_df = data.query("@select_param == @select_value")
    ...
    if st.sidebar.checkbox('Show the tree data'):
        cols = st.sidebar.multiselect('Select the columns to show in the table', data_columns)
        tableFrame.plotly_chart(showTable(select_df, cols))
    else:
        tableFrame.write('')
        
        
    if st.sidebar.checkbox('Map the tree data'):
    ...
    if st.sidebar.checkbox('Analyze Tree Diversity'):
        diversity(select_df)
